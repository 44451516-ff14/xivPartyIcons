...
public sealed class AppearanceTab
{
    ...
    private static void DrawDisplayConfig(DisplayConfig config, ref List<Action> actions)
    {
        ...
        using (var combo = ImRaii.Combo("交换样式", UiNames.GetName(config.SwapStyle))) {
            if (combo) {
                foreach (var style in Enum.GetValues<StatusSwapStyle>()) {
                    if (ImGui.Selectable(UiNames.GetName(style), style == config.SwapStyle)) {
        ...
        ImGuiComponents.HelpMarker(
            """
            确定如何对设置为"重要"的状态执行图标交换：
            - "无"：不执行任何操作
            - "交换"：将状态图标和职业图标位置互换
            - "替换"：将状态图标移动到职业图标槽位，状态图标槽位留空
            """);

        if (config.Mode is NameplateMode.SmallJobIconAndRole or NameplateMode.RoleLetters) {
            using var combo = ImRaii.Combo("角色显示样式", UiNames.GetName(config.RoleDisplayStyle));
            if (combo) {
        ...
        ImGuiExt.HoverTooltip("按住Control以允许重置");
        ...
        if (config.Preset == DisplayPreset.Custom) {
            ...
            ImGuiExt.HoverTooltip("按住Control以允许删除");
        }
